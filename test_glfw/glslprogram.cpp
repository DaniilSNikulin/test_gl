...
{
  compileShader( fileName, getShaderType(fileName) );
}
...
{
  compileShader(readFile(fileName), type, fileName);
}

void GLSLProgram::compileShader(const string & source, GLSLShaderType type, const char * fileName)
{
  if (handle <= 0) {
...
GLSLShaderType GLSLProgram::getShaderType(const char * fileName)
{
	int numExts = sizeof(GLSLShaderTypeInfo::extensions) / sizeof(GLSLShaderTypeInfo::shader_file_extension);
	// Check the file name's extension to determine the shader type
	string ext = getExtension(fileName);
	GLSLShaderType type = GLSLShaderType::VERTEX;
	bool matchFound = false;
	for (int i = 0; i < numExts; i++) {
		if (ext == GLSLShaderTypeInfo::extensions[i].ext) {
			matchFound = true;
			type = GLSLShaderTypeInfo::extensions[i].type;
			break;
		}
	}
...
string GLSLProgram::readFile(const char * fileName)
{
